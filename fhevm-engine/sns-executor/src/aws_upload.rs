...
async fn create_s3_client(conf: &Config) -> (Arc<aws_sdk_s3::Client>, bool) {
    ...
    let timeout_config = TimeoutConfig::builder()
        .disable_connect_timeout()
        .disable_read_timeout()
        .disable_operation_attempt_timeout()
        .operation_attempt_timeout(Duration::from_secs(60)) // per-attempt timeout
        .build();

    let retry_config = RetryConfig::standard()
        .with_max_attempts(s3config.retry.max_retries_per_upload)
        .with_max_backoff(s3config.retry.max_backoff);
