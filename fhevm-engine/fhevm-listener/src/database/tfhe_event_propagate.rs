...
impl Database {
    ...
    pub async fn find_tenant_id_or_panic(pool: &sqlx::Pool<Postgres>, tenant_api_key: &CoprocessorApiKey) -> TenantId {
        ...
        loop {
            match query().await {
                ...
                Err(SqlxError::RowNotFound) => panic!("No tenant found for the provided API key, please check your API key"),
    ...
    async fn insert_computation_bytes(
        ...
        let dependencies_handles = dependencies_handles.iter().map(|d| d.to_be_bytes_vec()).collect::<Vec<_>>();
        let dependencies = [
            &dependencies_handles,
            dependencies_bytes,
        ].concat();
        self.insert_computation_inner(tenant_id, result, dependencies, fhe_operation, scalar_byte).await
    }
    ...
    pub async fn insert_tfhe_event(
        ...
        match &event.data {
            E::Cast(C::Cast {caller, ct, toType, result})
            => self.insert_computation_bytes(tenant_id, result, &[ct], &[ty(toType)], fhe_operation, &HAS_SCALAR).await,

            E::FheAdd(C::FheAdd {caller, lhs, rhs, scalarByte, result})
            | E::FheBitAnd(C::FheBitAnd {caller, lhs, rhs, scalarByte, result})
            | E::FheBitOr(C::FheBitOr {caller, lhs, rhs, scalarByte, result})
            | E::FheBitXor(C::FheBitXor {caller, lhs, rhs, scalarByte, result} )
            | E::FheDiv(C::FheDiv {caller, lhs, rhs, scalarByte, result})
            | E::FheMax(C::FheMax {caller, lhs, rhs, scalarByte, result})
            | E::FheMin(C::FheMin {caller, lhs, rhs, scalarByte, result})
            | E::FheMul(C::FheMul {caller, lhs, rhs, scalarByte, result})
            | E::FheRem(C::FheRem {caller, lhs, rhs, scalarByte, result})
            | E::FheRotl(C::FheRotl {caller, lhs, rhs, scalarByte, result})
            | E::FheRotr(C::FheRotr {caller, lhs, rhs, scalarByte, result})
            | E::FheShl(C::FheShl {caller, lhs, rhs, scalarByte, result})
            | E::FheShr(C::FheShr {caller, lhs, rhs, scalarByte, result})
            | E::FheSub(C::FheSub {caller, lhs, rhs, scalarByte, result})
            => self.insert_computation(tenant_id, result, &[lhs, rhs], fhe_operation, scalarByte).await,

            E::FheIfThenElse(C::FheIfThenElse {caller, control, ifTrue, ifFalse, result})
            ...
            | E::FheGe(C::FheGe {caller, lhs, rhs, scalarByte, result})
            | E::FheGt(C::FheGt {caller, lhs, rhs, scalarByte, result})
            | E::FheLe(C::FheLe {caller, lhs, rhs, scalarByte, result})
            | E::FheLt(C::FheLt {caller, lhs, rhs, scalarByte, result})
            | E::FheNe(C::FheNe {caller, lhs, rhs, scalarByte, result})
            => self.insert_computation(tenant_id, result, &[lhs, rhs], fhe_operation, scalarByte).await,


            E::FheNeg(C::FheNeg {caller, ct, result})
            | E::FheNot(C::FheNot {caller, ct, result})
            => self.insert_computation(tenant_id, result, &[ct], fhe_operation, &NO_SCALAR).await,

            E::FheEqBytes(C::FheEqBytes {caller, lhs, rhs, scalarByte, result})
            | E::FheNeBytes(C::FheNeBytes {caller, lhs, rhs, scalarByte, result})
            => self.insert_computation_bytes(tenant_id, result, &[lhs], &[rhs.to_vec()], fhe_operation, scalarByte).await,

            E::FheRand(C::FheRand {caller, randType, seed, result})
            => self.insert_computation_bytes(tenant_id, result, &[], &[seed.to_vec(), ty(randType)], fhe_operation, &HAS_SCALAR).await,
            | E::FheRandBounded(C::FheRandBounded {caller, upperBound, randType, seed, result})
            ...
            | E::TrivialEncrypt(C::TrivialEncrypt {caller, pt, toType, result})
            => self.insert_computation_bytes(tenant_id, result, &[pt], &[ty(toType)], fhe_operation, &HAS_SCALAR).await,

            | E::TrivialEncryptBytes(C::TrivialEncryptBytes {caller, pt, toType, result})
...
fn event_to_op_int(op: &TfheContractEvents) -> FheOperation {
    ...
    match op {
        ...
        // Not tfhe ops
        E::Initialized(_) | E::OwnershipTransferStarted(_) | E::OwnershipTransferred(_) | E::Upgraded(_) | E::VerifyCiphertext(_) => -1,
    }
}
