...
async fn start_coprocessor(rx: Receiver<bool>, app_port: u16, db_url: &str) {
    let args: Args = Args {
        ...
        server_maximum_ciphertexts_to_schedule: 10000,
        server_maximum_ciphertexts_to_get: 10000,
        work_items_batch_size: 5000,
...
pub async fn decrypt_ciphertexts(
    ...
    let mut keys = sqlx::query!(
        "
            SELECT gpu_cks_key, gpu_csks_key
            FROM tenants
            WHERE tenant_id = $1
        ",
        tenant_id
    ...
    #[cfg(not(feature = "gpu"))]
    if keys[0].cks_key.is_none() {
        panic!("tenant keys not found");
    }
    #[cfg(feature = "gpu")]
    if keys[0].gpu_cks_key.is_none() {
        panic!("tenant keys not found");
    }
    ...
    let mut values = tokio::task::spawn_blocking(move || {
        ...
        #[cfg(feature = "gpu")]
        let cks = keys.gpu_cks_key.clone().unwrap();
        ...
        let sks = {
            let csks: tfhe::CompressedServerKey =
                safe_deserialize_key(&keys.gpu_csks_key.expect("missing csks key for GPU in DB"))
                    .unwrap();
            csks.decompress()
        };
