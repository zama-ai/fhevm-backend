...
describe('InputVerifier', function () {
  before(async function () {
    ...
    this.kmsFactory = await ethers.getContractFactory('InputVerifier');
  ...
  it('original owner adds one signer, then adds one more signers, then adds one more, then removes one signer', async function () {
    if (process.env.HARDHAT_PARALLEL !== '1') {
      ...
      const inputVerifier = await this.kmsFactory.attach(origIVAdd);
  ...
  it('cannot add/remove signers if not the owner', async function () {
    const origInputAdd = dotenv.parse(fs.readFileSync('addresses/.env.inputverifier')).KMS_VERIFIER_CONTRACT_ADDRESS;
    const inputVerifier = await this.kmsFactory.attach(origInputAdd);
